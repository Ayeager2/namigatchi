...
import "./fx.css";
import TamaMinigamePanel from "./TamaMinigamePanel";
...
export default function TamaPanel() {
  ...
  const [view, setView] = useState("tama"); // "tama" | "minigame"

  const [nowSec, setNowSec] = useState(Date.now());
  
  const disabled = !state.alive;
  
  const locked = disabled || state.sleeping || state.working;
  ...
  const closeMinigames = () => {
    if (state.alive && !state.sleeping) actions.resume();
    setView("tama");
  };

  useEffect(() => {
    if (!state.working) return;
    const id = setInterval(() => setNowSec(Date.now()), 250); // smooth-ish
  ...
  return (
    <div className={state.tantrum ? "fx-shake" : ""} style={{ maxWidth: 520, padding: 16, border: "1px solid #ddd", borderRadius: 14 , position: "relative"}}>
      ...
      {view === "tama" ? ( 
        ...
        {state.working && (
          <div style={{ fontSize: 12, opacity: 0.8 }}>
            Work ends in {Math.max(0, Math.ceil((state.workEndsAt - Date.now()) / 1000))}s
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
          ...
          <button
            className="fx-btn fx-btn--good"
            onClick={() => actions.goWork()}
            disabled={disabled || state.sleeping || state.working || !(state.stage === "child" || state.stage === "teen" || state.stage === "adult")}
          >
            {state.working ? "Working…" : "Go to Work"}
          </button>
       ...
       {!state.alive && (
          <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
            <input
              ...
              placeholder="Name your next egg"
            />
        ...
        <ShopPanel state={state} actions={actions} />
      </>
      ) : (
        <>
          <TamaMinigamePanel
